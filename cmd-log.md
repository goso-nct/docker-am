...
Hello, **from VOLUME**! Now: - 11.06.2021 14:40:34  
...
Удобно - имеем БД для отладки - не инсталили, не настраивали, получили из коробки (из образа)  

При разработке, запуск изменённой версии меняет верхний слой имаджа, а т.к. имя не изменяется, то оно переносится на новый контейнер, а старые остаются с \<none> - либо нужно каждый раз менять тэг.  
Контейнер с \<none> можно удалять по id.  
